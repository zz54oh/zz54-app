    ...
    function bindAll(mc) {
        ...
        if (clearBtn) clearBtn.addEventListener('click', function () {
            ...
            var doReset = function () {
                const _pledge = localStorage.getItem('splashPledgeSigned_v3');
                const _salt = localStorage.getItem('_dgUserSalt');
                const _hue = localStorage.getItem('CHAT_APP_V3_home_theme_hue');
                localStorage.clear();
                if (_pledge) localStorage.setItem('splashPledgeSigned_v3', _pledge);
                if (_salt) localStorage.setItem('_dgUserSalt', _salt);
                if (_hue) localStorage.setItem('CHAT_APP_V3_home_theme_hue', _hue);

                localforage.getItem('CHAT_APP_V3_tour_seen').then(function (val) {
                    return localforage.clear().then(function () {
                        if (val) return localforage.setItem('CHAT_APP_V3_tour_seen', val);
                    });
                }).then(function () {
