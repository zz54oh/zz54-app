document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // 恢复主页文字颜色设置
        localforage?.getItem('home_text_color').then(v => { const hs = document.getElementById('home-screen'); if(!hs) return; if (v === 'dark') { hs.style.setProperty('--home-text', '#1a1a1a'); hs.style.setProperty('--home-text-shadow', 'none'); } else if (v === 'light') { hs.style.setProperty('--home-text', '#ffffff'); hs.style.setProperty('--home-text-shadow', '0 1px 4px rgba(0,0,0,0.6)'); } }).catch(()=>{});
        // 字体大小slider
